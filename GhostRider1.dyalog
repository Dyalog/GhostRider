...
⍝ - SIStack, Threads, Status, Workspace Explorer, Process Manager
    ...
    ∇ (done output wins errors)←fn ProcessMessages messages;arguments;command;em;en;msg;win
      ...
      :For command arguments :In messages
          ...
          :CaseList 'OpenWindow' 'UpdateWindow'
              ...
              win.(title text line stop)←arguments.(name text currentRow stop)
    ...
    ⍝ Close an edit or tracer window
      ...
      :CaseList 'Editor' 'Tracer'
          ...
          :ElseIf ok∧(1=≢wins)∧(IsWin win) ⋄ :AndIf 'Tracer'≡(⊃wins).type  ⍝ edit turned back into a tracer
          :Else
      ...
      :EndSelect
    ∇
    ∇ CloseWindows;mask;messages;wins
    ⍝ Close all windows
      :Access Public
      Send'["CloseAllWindows",{}]'
      :If 0∊⍴WINS ⋄ wins←WINS ⋄ mask←⍬
      :Else ⋄ wins←(mask←WINS.type∊'Editor' 'Tracer')/WINS
      :EndIf
      :If 0∊mask ⋄ Reply¨(~mask)/WINS ⋄ :EndIf  ⍝ close dialog boxes
      ...
      WINS←NO_WIN  ⍝ message boxes implicitly discarded
    ∇
